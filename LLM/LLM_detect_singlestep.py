...
with open("LLM/Prompt/act_thought_analyse.txt", 'r', encoding='utf-8') as file:
...
def generate_inputs(pipe):
    ...
    for data in task:
        ...
        # 执行相似性搜索
        results_with_scores = vectorstore.similarity_search_with_score(data.request["user"], 6)
        ...
        final_input = prompt_analyse.format(input_text=input_text, retrive_attack_information=retrive_attack_vector)
        # 存储元数据以备后用
        metadata = {
